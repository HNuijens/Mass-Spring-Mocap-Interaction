...
PosInput3D[] input = new PosInput3D[15];     // Excitation inputs
ModelRenderer renderer;                      // Rendering
miString string,string2;                             // String
...
// Input
int nInput = 15;                                      // Number of input masses
...
void setup()
{
  ...
  //---Motion captures---//
  Mocap mocap1 = new Mocap("05_08.bvh");     // Load motion capture data:               
  // Constructor:
  ...
  string = new miString("string", med, 60, radius, m, k, z, 0.007, 0.003, "2D");
  string.rotate(0, PI/2, 0);
  string.translate(-1.9/10, 1.9/10, 0);
  string.changeToFixedPoint("m_0");                            // Ground 1
  string.changeToFixedPoint(string.getLastMass());             // Ground 2
  
  string2 = new miString("string2", med, 60, radius, m, k, z, dist, 0.003, "2D");
  string2.rotate(0, PI/2, 0);
  string2.translate(-1.7/10, 1.6/10, -2./10);
  string2.changeToFixedPoint("m_0");                            // Ground 1
  string2.changeToFixedPoint(string2.getLastMass());            // Ground 2
  ...
  phys.mdl().addInOut("listener2", new Observer3D(filterType.HIGH_PASS), phys.mdl().getPhyModel("string2").getMass("m_10"));
  phys.colEngine().addCollision(string, perc, 0.0002, -0.01);   // Bowing settings
  phys.colEngine().addCollision(string2, perc, 0.0002, -0.008);   // Bowing settings
  //phys.colEngine().addCollision(string, perc, 0.05, 0.01);    // Plucking settings
  //phys.colEngine().addCollision(string2, perc, 0.05, 0.01);    // Plucking settings
...
void draw()
{
  ...
  if (k > 0.1 && k<0.9)string2.setParam(param.STIFFNESS, k); // ensure it doesnt break
